...
from DBScan import global_vars as g

g.settings = g.Settings()
import pyqtgraph as pg
from DBScan.FileReader import file_to_array
import time
from sklearn.cluster import DBSCAN
from DBScan.ClusterMath import *
...
class Cluster():
    ...
    def set_z_points(self, xyzs, color):
        for point in self.points:
            for coord in xyzs:
                if point[0] == coord[0] and point[1] == coord[1]:
                    self.zs.append(coord[2])
                    self.color = color
        self.ZCenter = getZCenter(self.zs)
